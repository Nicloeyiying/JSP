...
<div id="content" class="wrap">
	<div class="list bookList">
		<form method="post" name="shoping" action="http://localhost:8080/book8/addCart.do">
			<table>
				<tr class="title">
					<th class="checker"></th>
					<th>书名</th>
				...
				<c:if test="${prods == null}">
				   <c:redirect url="http://localhost:8080/book8/productServlet.do"/>
				</c:if>
				<c:forEach items="${prods}" var="pr">
				 <tr>
					<td><input type="checkbox" name="bookId" value="${pr.id}" /></td>
					<td class="title">${pr.name}</td>
					<td>￥${pr.price}</td>
					<td>${pr.price}</td>
					<td class="thumb">
					 <img src="${pr.imgurl}" width="100px" height="100px" /></td>
				 </tr>
			  </c:forEach>
			</table>
			...
			<div class="button">
			<input class="input-btn" type="submit" name="submit" value="" /></div>
		</form>
	</div>
</div>
